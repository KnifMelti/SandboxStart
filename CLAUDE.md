...
### WinGet Configuration Files (.winget)

WinGet Configuration (DSC) files provide declarative package installation with advanced features like dependency management and configuration-as-code.

**Usage:** Select a `.winget` file in the file browser. Windows automatically executes the configuration via the WinGet file association when the file is opened in the sandbox.
